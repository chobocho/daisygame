...
class DrawEngine {
  ...
  private _drawArrowButton(cx: number, cy: number, glyph: string, enabled: boolean): void {
    bufCtx.save();
  ...
  private _hitLevelSelect(x: number, y: number): number {
    ...
    const py = 305 * gScale;
    const r = 24 * gScale;
    if ((x - px) ** 2 + (y - py) ** 2 <= r ** 2) return NAV_PREV_KEY;
    // Next arrow at (330, 305) r=24
    const nx = gStartX + 330 * gScale;
    if ((x - nx) ** 2 + (y - py) ** 2 <= r ** 2) return NAV_NEXT_KEY;
    ...
    if (x > pbx1 && x < pbx2 && y > pby1 && y < pby2) return S_KEY;
