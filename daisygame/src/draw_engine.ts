...
class DrawEngine {
  ...
  private static readonly PUZZLE_TIMER_PALETTES: ReadonlyArray<PetalColor> = (() => {
    const N = 12;
    const RED:   [number, number, number] = [196, 43, 43];    // #C42B2B
    const WHITE: [number, number, number] = [255, 255, 255];
    const BLUE:  [number, number, number] = [43, 79, 196];    // #2B4FC4

    // 3-stop interpolation: RED at t=0, WHITE at t=0.5, BLUE at t=1.
    const lerp3 = (t: number): [number, number, number] => {
      const blend = (a: [number, number, number], b: [number, number, number], u: number): [number, number, number] => [
    ...
    const palette = (rgb: [number, number, number]): PetalColor => {
      const [r, g, b] = rgb;
      const fmt = (rr: number, gg: number, bb: number) => `rgb(${rr}, ${gg}, ${bb})`;
      return {
        base: fmt(r, g, b),
        light: fmt(
  ...
  private _drawButton(): void {
    if (this.game.isIdleState()) {
      ...
      this._drawTimer();
      if (this.game.mode() === 1) this._drawPuzzleHud();
    }
  }
  ...
  private static readonly GRID_X = 20;
  private static readonly GRID_Y = 100;
  private static readonly GRID_COLS = 10;
  private static readonly GRID_ROWS = 10;
  private static readonly GRID_CELL_W = 36;
  ...
  private _drawLevelCell(
    cx: number,
    cy: number,
    level: number,
    prog: PuzzleProgressLike | null,
    isUnlocked: boolean,
    isSelected: boolean,
  ): void {
  ...
  private _drawStars(cx: number, cy: number, filled: number, size: number): void {
  ...
  private _drawStar(cx: number, cy: number, r: number, filled: boolean): void {
    bufCtx.save();
  ...
  private _drawPuzzlePetalTimer(totalSec: number): void {
  ...
  private _drawPuzzleGameOver(): void {
    const lvl = this.game.puzzleLevel();
    const cfg = Puzzle.levelConfig(lvl);
  ...
  getEventCode(x: number, y: number): number {
    ...
    if (this.game.isIdleState()) {
      ...
      if (isPuzzle) {
        ...
        if (x > lbx1 && x < lbx2 && y > lby1 && y < lby2) return LEVEL_SELECT_KEY;
      }
  ...
  private _hitLevelSelect(x: number, y: number): number {
    ...
    if (tapped > 0) return DrawEngine.LEVEL_TAP_BASE + tapped;
  ...
  private _hitGridCell(x: number, y: number): number {
    ...
    if (col < 0 || col >= DrawEngine.GRID_COLS) return 0;
    if (row < 0 || row >= DrawEngine.GRID_ROWS) return 0;
    return row * DrawEngine.GRID_COLS + col + 1;
  }

  static readonly LEVEL_TAP_BASE = 1000;

  private _hitPuzzleGameOver(x: number, y: number): number {
    const lvl = this.game.puzzleLevel();
    const cfg = Puzzle.levelConfig(lvl);
    ...
    let levelSelectY: number;
    let menuY: number;
    if (cleared && lvl < Puzzle.MAX_LEVEL) {
      // Next at (200, 250)
      let by1 = (250 - halfH) * gScale;
      let by2 = (250 + halfH) * gScale;
      if (x > bx1 && x < bx2 && y > by1 && y < by2) return NEXT_LEVEL_KEY;
      // Retry at (200, 305)
    ...
    if (x > bx1 && x < bx2 && y > lby1 && y < lby2) return LEVEL_SELECT_KEY;
