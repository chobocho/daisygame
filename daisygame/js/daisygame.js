class DaisyGame {
    ...
    _fillEmptySlots() {
        // Use Flower.addLeaf so the freshly placed color is re-rolled away
        // from any already-alive _leafMap neighbor — keeps the auto-refill
        // from handing out free instant matches the player didn't earn.
        this._flowerArr.forEach(f => {
            for (let attempt = 0; attempt < 6 && f.leaf_count() < 6; attempt++) {
    ...
    increaseTick() {
        ...
        // Puzzle mode keeps the board topped up every tick so the player
        // isn't waiting between matches. Only color === 0 slots are
        // touched, so dying leaves (life > 0) and rainbow / gold leaves
        // sit through the auto-refill untouched.
        if (this._mode === this.MODE_PUZZLE) {
            this._fillEmptySlots();
        }
